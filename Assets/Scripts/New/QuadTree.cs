...
namespace Kultie.NewQuadTree
{
    ...
    public class QuadTree
    {
        ...
        static int enode_idx_next = 0;

        // Stores the element index.
        static int enode_idx_elt = 1;
        ...
        static int elt_idx_lft = 0, elt_idx_top = 1, elt_idx_rgt = 2, elt_idx_btm = 3;

        // Stores the ID of the element.
        static int elt_idx_id = 4;
        ...
        static int nd_idx_mx = 0, nd_idx_my = 1, nd_idx_sx = 2, nd_idx_sy = 3;

        // Stores the index of the node.
        static int nd_idx_index = 4;

        // Stores the depth of the node.
        static int nd_idx_depth = 5;
        ...
        public QuadTree(int width, int height, int start_max_elements, int start_max_depth)
        {
        ...
        {
            // Find the leaves.
            int lft = elts.get(element, elt_idx_lft);
            int top = elts.get(element, elt_idx_top);
            int rgt = elts.get(element, elt_idx_rgt);
            int btm = elts.get(element, elt_idx_btm);
            ...
            for (int j = 0; j < leaves.size(); ++j)
            {
                ...
                if (node_index != -1)
                {
                    ...
                    int next_index = enodes.get(node_index, enode_idx_next);
                    if (prev_index == -1)
        ...
        public void cleanup()
        {
            ...
            while (to_process.size() > 0)
            {
                ...
                int node = to_process.get(to_process.size() - 1, 0);
                int fc = nodes.get(node, node_idx_fc);
        ...
        public IntList query(float x1, float y1, float x2, float y2, int omit_element)
        {
            ...
            for (int j = 0; j < leaves.size(); ++j)
            {
                ...
                {
                    int element = enodes.get(elt_node_index, enode_idx_elt);
                    int lft = elts.get(element, elt_idx_lft);
                    int top = elts.get(element, elt_idx_top);
                    int rgt = elts.get(element, elt_idx_rgt);
                    int btm = elts.get(element, elt_idx_btm);
                    ...
                    {
                        _tmp.set(_tmp.pushBack(), 0, element);
                        temp[element] = true;
                    }
        ...
        public void traverse(IQtVisitor visitor)
        {
            ...
            while (to_process.size() > 0)
            {
                int back_idx = to_process.size() - 1;
                int nd_mx = to_process.get(back_idx, nd_idx_mx);
                int nd_my = to_process.get(back_idx, nd_idx_my);
                int nd_sx = to_process.get(back_idx, nd_idx_sx);
                int nd_sy = to_process.get(back_idx, nd_idx_sy);
                int nd_index = to_process.get(back_idx, nd_idx_index);
                int nd_depth = to_process.get(back_idx, nd_idx_depth);
                int fc = nodes.get(nd_index, node_idx_fc);
                to_process.popBack();
                ...
                if (nodes.get(nd_index, node_idx_num) == -1)
                {
                    ...
                    int l = nd_mx - hx, t = nd_my - hx, r = nd_mx + hx, b = nd_my + hy;
        ...
        private void leaf_insert(int node, int depth, int mx, int my, int sx, int sy, int element)
        {
            ...
            int nd_fc = nodes.get(node, node_idx_fc);
            ...
            if (nodes.get(node, node_idx_num) == max_elements && depth < max_depth)
            {
                ...
                {
                    int index = nodes.get(node, node_idx_fc);
                    int next_index = enodes.get(index, enode_idx_next);
                    int elt = enodes.get(index, enode_idx_elt);
